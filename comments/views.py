...
from blog.models import Post,Essay

from .models import Comment,Essay_Comment
from .forms import CommentForm,Essay_CommentForm
...
def essay_comment(request, essay_id):
    # 先获取被评论的文章，因为后面需要把评论和被评论的文章关联起来。
    # 这里我们使用了 Django 提供的一个快捷函数 get_object_or_404，
    # 这个函数的作用是当获取的文章（Essay）存在时，则获取；否则返回 404 页面给用户。
    ...
    if request.method == 'POST':
        ...
        form = Essay_CommentForm(request.POST)
